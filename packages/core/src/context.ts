 ...
 */
    ...
    async searchWithLayers(
        ...
        if (isHybrid && activeLayers.length > 1 && typeof this.vectorDatabase.sparseSearch === 'function') {
            ...
            const fused = await this.globalHybridFusion(layerObjs, queryEmbedding.vector, query, fetchK, filterExpr, threshold);
        ...
        const perLayer = await Promise.all(
            activeLayers.map(layer => {
                ...
                return this.searchLayer(
                    layer.collectionName, queryEmbedding.vector, query, fetchK, threshold, layerFilter, isHybrid,
                ).catch(error => {
